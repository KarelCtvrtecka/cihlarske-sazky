...
<style>
    ...
    .bet-card { background: white; border-radius: 12px; padding: 10px; text-align: center; border: 2px solid #eee; box-shadow: 0 4px 6px rgba(0,0,0,0.05); position: relative; height: 150px; display: flex; flex-direction: column; justify-content: center; align-items: center; }
...
# --- GOOGLE SHEETS FUNKCE ---
def get_sheet():
    # Tady je ten trik s povolením disku, co nám chyběl
...
if not st.session_state.user:
    ...
    if page == "DOMŮ":
        ...
        if data["market"]["status"] == "OPEN":
            if not user["pay"]:
                if st.button("💸 Vybrat výplatu"):
                    inc = RANKS[rid]["inc"]; user["bal"] += inc; user["pay"] = True
            ...
            if user["hp"] != "OK": st.error("🤕 Jsi zraněn! Nemůžeš sázet.")
                ...
                for c_name, odd in data["market"]["colors"].items():
                    with cols[idx % 4]:
                        hex_c = COLORS.get(c_name, "#ccc"); card_style = ""; extra_info = ""
                        ...
                        if "original_odds" in data["market"] and c_name in data["market"]["original_odds"]:
                            if odd > data["market"]["original_odds"][c_name]:
                                card_style = "border: 2px solid #ffd700; box-shadow: 0 0 15px #ffd700;"; extra_info = f"<br><span style='color:#ffd700;font-weight:bold;font-size:0.9em'>⚡ MEGA</span>"
                ...
                if "target" in st.session_state:
                    tc, to = st.session_state["target"]; st.info(f"Tiket: **{tc}** (Kurz {to})")
                    vklad = st.number_input("Vklad", 1, max(1, int(user["bal"])), 50)
        ...
        if pending:
            for b in pending: st.markdown(f"<div class='ticket-pending'><b>{b['c']}</b> | {b['a']} CC</div>", unsafe_allow_html=True)
        else: st.caption("Žádné aktivní sázky.")
        with st.expander("📜 Historie"):
            for b in reversed([x for x in user["bets"] if x["st"] != "PENDING"]):
                cls = "ticket-won" if b["st"] == "WON" else "ticket-lost"; res = "✅" if b["st"] == "WON" else "❌"
                st.markdown(f"<div class='{cls}'>{res} <b>{b['c']}</b> ({b['a']} CC)</div>", unsafe_allow_html=True)

    elif page == "ŽEBŘÍČEK":
        st.title("🏆 Žebříček")
        for i, (tn, td) in enumerate(sorted(data["users"].items(), key=lambda x: x[1]['bal'], reverse=True)):
            hp = "🤕" if td["hp"] != "OK" else ""; strk = f"🔥 {td['streak']}" if td.get('streak',0)>0 else ""
            st.markdown(f"<div style='background:white;padding:10px;border-radius:5px;margin-bottom:5px;border-left:3px solid #ccc'><b>#{i+1} {tn}</b> {hp} {strk} <div style='float:right;font-weight:bold'>{int(td['bal'])} CC</div></div>", unsafe_allow_html=True)
        ...
        c1, c2, c3 = st.columns(3); c1.markdown(f"<div class='stat-box'><div class='stat-label'>Příjem</div><div class='stat-val'>+{s['total_income_all']}</div></div>", unsafe_allow_html=True); c2.markdown(f"<div class='stat-box'><div class='stat-label'>Výhry</div><div class='stat-val'>+{s['total_bet_winnings']}</div></div>", unsafe_allow_html=True); c3.markdown(f"<div class='stat-box'><div class='stat-label'>Útrata</div><div class='stat-val'>-{s['total_spent']}</div></div>", unsafe_allow_html=True)
        c4, c5, c6 = st.columns(3); fav = max(s['color_counts'], key=s['color_counts'].get) if s['color_counts'] else "Žádná"; c4.markdown(f"<div class='stat-box'><div class='stat-label'>Oblíbená</div><div class='stat-val' style='color:{COLORS.get(fav, '#333')}'>{fav}</div></div>", unsafe_allow_html=True); c5.markdown(f"<div class='stat-box'><div class='stat-label'>W/L</div><div class='stat-val'>{s['total_wins']}/{s['total_losses']}</div></div>", unsafe_allow_html=True); c6.markdown(f"<div class='stat-box'><div class='stat-label'>Max Win</div><div class='stat-val'>{s['max_win']}</div></div>", unsafe_allow_html=True)
        ...
        with t1:
            if user["rank"] < 5:
                ...
                if st.button(f"Koupit hodnost ({p} CC)"):
        ...
        with t2:
            ...
            for item in data["shop"]:
                p = item["curr_p"]
                if "Kbelík" in item["name"]: p = item["base_p"] + (user["slots"] * 2000)
                c1, c2 = st.columns([3,1]); c1.markdown(f"**{item['name']}** ({p} CC)"); c1.caption(item['desc'])
        ...
        for i, item in enumerate(user["inv"]):
            c1, c2 = st.columns([3,1]); c1.write(f"📦 {item}")
            if "Svačina" in item and c2.button("Použít", key=f"u{i}"): user["bal"]+=50; user["inv"].pop(i); save_data(data); st.rerun()
            if ("Cihla" in item or "BOZP" in item) and c2.button("Aktivovat", key=f"u{i}"): user["bonus"]=item; user["inv"].pop(i); save_data(data); st.rerun()
        ...
        st.divider(); 
        for t in reversed(user["trans"]): st.markdown(f"⬇️ +{t['amt']} ({t['src']})" if t['type']=='in' else f"⬆️ -{t['amt']} ({t['src']})")

    elif page == "CHAT":
        st.title("📢 Chat")
        with st.container():
            for m in data["chat"][-50:]: 
                cls = "msg-sys" if m['u'] in ["SYS","EVENT","BANKA","SHOP"] else "msg-user"; st.markdown(f"<div class='{cls}'><small>{m['tm']}</small> <b>{m['u']}</b>: {m['t']}</div>", unsafe_allow_html=True)
        with st.form("cf"): 
    ...
    elif page == "📚 NÁPOVĚDA": st.title("📚 Manuál"); st.write("Výplata každý den. Sázej na barvy. Kupuj bonusy v obchodě. Znič soupeře v žebříčku.")
        ...
        if not st.session_state.get("admin_ok"): 
            ...
            with t4:
                st.info("Reset databáze prováděj přes Google Tabulku (smaž buňku A1 a napiš '{}')")
