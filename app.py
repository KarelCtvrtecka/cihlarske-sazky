...
def load_data():
    ...
    if not os.path.exists(DATA_FILE): return base
    try:
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            ...
            # --- SAFEGUARD: Oprava zaseknutých kurzů při startu ---
            # Pokud je trh zavřený, ale kurzy jsou nesmyslné (zbylo z pádu), resetuj je na 2.0
            if d["market"].get("status") == "CLOSED":
                for c in d["market"]["colors"]:
                    if d["market"]["colors"][c] > 9.0: # Detekce zaseknutého Mega Eventu
...
def trigger_game_event(data, event_type):
    msg = ""
    if event_type == "MEGA":
        # SAFEGUARD: Zálohovat jen pokud není záloha. Nikdy nepřepisovat zálohu, pokud už existuje.
        ...
        for bc in boosted: 
            # Vždy počítat z ORIGINÁLNÍHO kurzu, nikdy z aktuálního
